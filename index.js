...
function findLastEvenNumber(arr) {
    ...
    if (!Array.isArray(arr)) {
        ...
        let num = arr.findLast((element) => element % 2 == 0);
        return num !== undefined ? num : "Парних чисел не знайдено";
    }
}
...
function sumAndPrint(arr) {
    ...
    if (Array.isArray(arr)) {
        let sum = arr.reduce((sum, currentElement) => sum + currentElement, 0);
...
function getLastElementsSumAndPrint(arr, n) {
    ...
    if (Array.isArray(arr)) {
        ...
        return newArr.reduceRight((sum, element) => (sum += element), 0);
    } else {
        return "Вхідний параметр має бути масивом";
    }
}
...
function findLongestWord(arr) {
    ...
    if (Array.isArray(arr)) {
        let length = arr.reduce(
            (len, element) =>
                element.length > len ? (len = element.length) : len,
            0
        );
        return arr.find((element) => element.length == length);
    } else {
        return "";
    }
    // Версія 2
...
function calculateTotalPrice(arr) {
    ...
    if (Array.isArray(arr)) {
        return arr.reduce((sum, element) => (sum += element.price), 0);
    } else {
        return 0;
    }
}
